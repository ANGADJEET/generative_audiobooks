...
class AdaIN(nn.Module):
    ...
    def forward(self, x, style):
        style = self.fc(style).unsqueeze(-1)  # [B, 2*C, 1]
...
class StochasticDurationPredictor(nn.Module):
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout, n_flows=4, 
        ...
        if emotion_embedding_dim > 0:
            self.emotion_proj = nn.Linear(emotion_embedding_dim, filter_channels)
    ...
    def forward(self, x, x_mask, w=None, g=None, reverse=False, noise_scale=1.0, emotion_emb=None):
        ...
        # Process both speaker and emotion embeddings
        if g is not None:
            g = torch.detach(g)
            x = x + self.cond(g)
        
        if emotion_emb is not None and self.emotion_proj is not None:
            emotion_proj = self.emotion_proj(emotion_emb).unsqueeze(-1)
            x = x + emotion_proj
...
class TextEncoder(nn.Module):
    def __init__(self, n_vocab, out_channels, hidden_channels, filter_channels, 
        ...
        self.emotion_proj = None
        if emotion_embedding_dim > 0:
            self.emotion_proj = nn.Linear(emotion_embedding_dim, hidden_channels)
...
class ResidualCouplingLayer(nn.Module):
    def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers,
                 p_dropout=0, gin_channels=0, mean_only=False, emotion_embedding_dim=0):
        super().__init__()
        ...
        self.post = nn.Conv1d(hidden_channels, channels * (2 if not mean_only else 1), 1)
        self.post.weight.data.zero_()
        self.post.bias.data.zero_()

    def forward(self, x, x_mask, g=None, emotion_emb=None, reverse=False):
        x0, x1 = torch.split(x, [self.channels//2, self.channels//2], 1)
        h = self.pre(x0) * x_mask
        h = self.enc(h, x_mask, g=g, emotion_emb=emotion_emb)
        stats = self.post(h) * x_mask
        if not self.mean_only:
            m, logs = torch.split(stats, self.channels//2, dim=1)
        else:
            m = stats
